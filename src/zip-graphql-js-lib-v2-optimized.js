...
class RewstApp {
  ...
  /**
   * Get a specific org variable by name with org ownership info.
   * Bypasses the 500-item ceiling on getOrgVariablesWithOrg() by targeting
   * the variable directly. Tries a scoped orgVariables query first; if the API
   * doesn't support that shape, falls back to a high-limit visibleOrgVariables
   * scan so the caller always gets a reliable answer.
   *
   * Fixes a real bug: in orgs with >500 total variables, `roadmap_data` (or
   * whatever target variable) could sit past the cutoff and never be returned,
   * causing the load logic to think it didn't exist and re-seed on every page
   * load — wiping the customer's data each refresh.
   *
   * @param {string} name - Exact variable name to look up
   * @param {string|null} targetOrgId - Owning org ID (default: current org)
   * @returns {Promise<Array>} Array of matching variable objects (usually 0 or 1)
   */
  async getOrgVariablesWithOrgByName(name, targetOrgId = null) {
    if (!this.isInitialized) {
      const error = new Error('Rewst not initialized. Call rewst.init() first!');
      this._error('getOrgVariablesWithOrgByName called before initialization', error);
      throw error;
    }
    if (!name) throw new Error('Variable name is required');

    const orgIdToUse = targetOrgId || this.orgId;
    this._log('Fetching org variable by name:', name, 'orgId:', orgIdToUse);

    // Primary: scoped query that returns only variables with this name for this org.
    // If the Rewst API supports it, this is O(1) regardless of total variable count.
    try {
      const query = `
        query getOrgVariableByName($orgId: ID!, $name: String!) {
          orgVariables(orgId: $orgId, name: $name) {
            id
            name
            value
            category
      ...
      `;
      const result = await this._graphql('getOrgVariableByName', query, {
        orgId: orgIdToUse,
        name,
      });
      if (result?.orgVariables !== undefined) {
        const vars = Array.isArray(result.orgVariables) ? result.orgVariables : [];
        this._log(`getOrgVariableByName: found ${vars.length} variable(s) named "${name}"`);
        return vars;
      }
    } catch (err) {
      // API doesn't support this query shape — fall through to scan
      this._log('Scoped orgVariables query unavailable, falling back to high-limit scan:', err.message);
    }

    // Fallback: fetch a large page of visibleOrgVariables and filter by name + owning org.
    // 5000 covers nearly all real orgs; if it still misses, the caller will seed (same
    // behavior as before) but this is far less likely to trigger than the 500-item cap.
    const fallbackLimit = 5000;
    this._log(`getOrgVariablesWithOrgByName fallback: scanning up to ${fallbackLimit} visible variables`);
    const fallbackQuery = `
      query getVisibleOrgVariables($visibleForOrgId: ID!, $limit: Int) {
        visibleOrgVariables(visibleForOrgId: $visibleForOrgId, limit: $limit) {
          id
    ...
    `;
    const fallbackResult = await this._graphql('getVisibleOrgVariables', fallbackQuery, {
      visibleForOrgId: orgIdToUse,
      limit: fallbackLimit,
    });
    const all = fallbackResult?.visibleOrgVariables || [];
    const matches = all.filter(v => v.name === name && v.organization?.id === orgIdToUse);
    this._log(`getOrgVariablesWithOrgByName fallback: found ${matches.length} match(es) in ${all.length} variables`);
    return matches;
  }
  ...
  async getOrgVariable(name) {
    ...
    try {
      // Use the scoped name-targeted lookup so we find the variable even when
      // getOrgVariables()'s default 100-item cap would miss it. (The old
      // implementation called getOrgVariables() and filtered by name client-
      // side — in orgs with >100 variables the target could sit past the
      // cutoff and silently return null. Same bug shape as the roadmap_data
      // load issue from a real customer environment.)
      const matches = await this.getOrgVariablesWithOrgByName(name, this.orgId);
      const variable = matches.find(v => v.organization?.id === this.orgId) || null;
